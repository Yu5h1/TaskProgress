...
> 這份文件只記錄設計：要做什麼、為什麼這樣做、驗收條件。**實作進度、核定日期、擱置與待決策一律看 `handoff.md`**，不在這裡重複。
>
> 一項決定尚未拍板時，它是 `handoff.md` 的 Open decision；拍板之後就成為設計，寫進這裡並從那份清單移除。因此本文件只在設計新增、修改或刪除時才會變動。
...
- 模組自己宣告它讀哪些上游 module type，依賴是軟的——上游在就讀、不在就忽略，並在畫面上指名該模組且說明其值未計入。上游變動只標記 dirty，重算依拓樸順序統一發生。契約全文見 `Documentation/ExtensionModuleArchitecturePlan.md#模組依賴與重算2026-08-28-使用者決策`。
- time 先遷移為第一個正式模組，再以 cost 作為第二個模組，驗證 project／task／item 對應、商業資料隱私及跨模組分析，而不讓核心新增成本特例。Cost 與 Time 同級；可明確讀取時間估算作為分析輸入，但投影與 Renderer 必須獨立載入、驗證及降級。第一版成本語意區分 actual、committed、estimated 與 replacement，並保存幣別、基準日、分類、分攤方法、來源及 confidence；付款與交易仍由外部系統負責。2026-08-28 起，材料與人工各自是獨立模組而非 Cost 的內建分類，估價模組讀取工時與人工的計價方式後執行計算；邊界與初版試跑結果見 `Documentation/CostEstimationModulePlan.md#邊界修訂2026-08-28-使用者決策`。
...
- 指路卡不快取目標 report。每次需要投影就重新讀取，讓畫面與來源保持一致；來源永遠是目標 report，不引入第二份會過期的副本。
