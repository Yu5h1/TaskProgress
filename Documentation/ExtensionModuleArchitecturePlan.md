...
**第三種形式：任務層是 label，不是膠囊（2026-08-25 使用者決策）。** 判準是**有沒有可操作的東西**：專案層有可設定的值（交付日、容量），子項層有可設定的值（人工工時），所以兩者是膠囊、點擊開啟面板。但**任務層是純衍生值**——它就是子項的加總，那一層沒有任何可編輯的欄位，因此做成看起來可按的控制項會承諾一個不存在的動作。任務層的模組貢獻掛在 `task-body`，以 label 呈現（`interactive: false`、`sortable: false`），排在任務標題下方自成一列，不與標題爭寬度。這也表示 `task-detail` 面板目前沒有使用者——任務卡上已經列出它的子項與各自的膠囊，再開一個任務面板只是把同一份資訊再顯示一次。
