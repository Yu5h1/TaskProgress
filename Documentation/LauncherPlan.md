...
2. 驗證報告的 scope、版本與配對關係。
3. 確認指定 port 是否已有正確的 LocalWebService。
4. 服務不存在時啟動它；已存在時重複使用同一個 process。
5. 將該 scope 的兩個 JSON 註冊為精確的 URL 路徑。
6. 開啟對應的 Viewer URL。

Viewer 繼續使用現有 `?scope=` 與 `?dev=` 契約，不需修改。
...
註冊選用的 /reports/<scope>/report.dev.json
    ↓
開啟 http://127.0.0.1:8001/?scope=<scope>&dev=...
```
...
```text
?scope=bonghuo-vr&dev=reports/bonghuo-vr/report.dev.json
```
...
- README 已更新；TaskProgress report skill 依最終捷徑使用方式另行同步。
