...
2. 驗證所有已登記資料夾的 `report.json` 與選用的 `report.dev.json`。
3. 在獨立 Console 啟動 LocalWebService。
4. 註冊所有 scope 的精確報告 URL。
5. 產生不包含本機路徑的 scope catalog。
6. 開啟 `http://127.0.0.1:8001/`，由首頁選擇報告。
...
1. 驗證 `report.json` 與選用的 `report.dev.json`。
2. 檢查 `127.0.0.1:8001`。
