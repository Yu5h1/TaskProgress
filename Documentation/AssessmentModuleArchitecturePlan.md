...
**適用範圍：readout，也就是面板最上方那一列「標籤 + 值」。** 它整列滿版，一個面板只有一個。

在這一列，**標籤與值永遠同列，任何寬度都不得改成標籤在上、值在下**。Unity Inspector、WPF property grid 與一般帳號密碼表單都是這個讀法：標籤靠左、值靠右，掃視一整欄最快。把標籤堆到值上面會放棄這個對齊，換不到任何東西。

**這條規則不適用於多欄磚格。** `AssessmentMetricGrid`（`minmax(130px, 1fr)` 自動填充）與 Time 專案總覽的三欄 `time-report-grid` 都是磚，一塊磚放不下左右並排的標籤與值，它們維持標籤在上。判準是版面情境而不是哪個面板：**滿版單列 → 標籤在左；多欄磚 → 標籤在上。**

子項面板與模組主面板的資料形狀差很多——前者是單一主體的少數屬性，後者是一組彙總數字——所以它們本來就不該套同一種版面。共用的是 readout 這一列的規則，不是整個面板的版面。
