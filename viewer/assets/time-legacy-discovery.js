/*
 * Legacy Time discovery/loading, extracted from `app.js`'s report-load flow
 ...
 * `time-viewer-module.js` and is already live). This is the "legacy
 * discovery adapter" the architecture plan describes: it finds and validates
 * the bare `time.analysis.json` sidecar by its known filename, the way every
 * report has always worked, as opposed to a future manifest-declared module
 * that would read a `report.modules.json` entry instead. `time.analysis.json`
 ...
 *
 * Same fixture → passive-shadow → cutover shape as the render layer:
 * `fetchJson` is injected so this stays testable without a real network call,
 * and this file performs no DOM access itself. Constructing
