...
var Oo = 1, ko = /* @__PURE__ */ new Set([
	"load",
	"save",
	"reset"
]);
function Ao({ scope: e, task: t, fetchImpl: n = globalThis.fetch, apiRoot: r = "/__taskprogress/v1" } = {}) {
	...
	async function a(a, o) {
		if (!ko.has(a)) throw Error(`不支援的 Checklist request：${a}`);
		let s = {
			version: Oo,
...
function Mo({ windowTarget: e = globalThis.window, documentTarget: t = globalThis.document, canRefresh: n = () => !0, reload: r = () => e.location.reload(), schedule: i = (e) => globalThis.queueMicrotask(e) } = {}) {
	if (jo(e, "windowTarget"), jo(t, "documentTarget"), typeof n != "function") throw TypeError("canRefresh 必須是函式。");
...
var Po = null;
Mo({ canRefresh: () => !Po?.dirty && !Po?.saving && !Po?.pending }), Ar(Do, {
